...
class PremiumGameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let backButton = SpriteKitButton(defaultButtonImage: "backButtonForScene", action: goBack, index: 0)
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if bird.grabbed{
            ...
            bird.flying = true
            roundState = .flying
            constrainstToAnchor(active: false)
    ...
    func setupLevel(){
        //having tilemap property to access tilemap node if present
    ...
    func addBird(){
        ...
        //bird should not move further far from anchor
        constrainstToAnchor(active: true)
        roundState = .ready
    }
    ...
    func createEnemy(from placeholder: SKSpriteNode, name: String) -> Enemy? {
        ...
        enemy.createPhysicBody()
        return enemy
    }
    
    func constrainstToAnchor(active: Bool){
        if active {
...
extension PremiumGameScene: PopupButtonHandlerDelegate {
    ...
    func nextTapped() {
        if let level = level {
            sceneManagerDelegate?.presentPremiumGameSceneFor(level: level + 1)
        }
    }
...
extension PremiumGameScene : SKPhysicsContactDelegate{
